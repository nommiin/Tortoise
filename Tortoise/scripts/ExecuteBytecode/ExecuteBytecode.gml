...
/// @argument code
var Environment = argument[0], Code = argument[1], Bytecode = Code[? "Bytecode"], Mapping = []; GenerateMap(Mapping);
buffer_seek(Bytecode, buffer_seek_start, 0);
while (buffer_tell(Bytecode) < buffer_get_size(Bytecode)) {
	ExecuteInstruction(Environment, Bytecode, Mapping, buffer_read(Bytecode, buffer_u32));
